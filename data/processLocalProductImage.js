...
/**
 * Processes a local image file and updates the corresponding product in the DB.
 * Maintains transparency for WebP/AVIF and adds #F4F4F6 for JPEGs.
 */
export const processLocalProductImage = async (inputFilePath, productId, deleteSource = false) => {
  try {
    ...
    // 2. Verify the product exists in the DB
    ...
    // 3. Generate Large JPEG (Flattened)
    ...
    // Update DB with the primary image immediately
    ...
    // 4. Generate variants using inputFilePath to maintain transparency for WebP/AVIF
    await Promise.all([
      sharp(inputFilePath)
        .resize(1600)
        .webp({ quality: 80 })
    ...
    // 6. Cleanup Source File (Only now that all processing is done)
    if (deleteSource) {
      await fs.unlink(inputFilePath).catch(err => console.error("Cleanup error:", err));
    }

    console.log(`🎉 All image variants successfully generated and linked to product ${productId}!`);
    return finalProduct;
